...
from utils.theme import COLOR_ON_PRIMARY, COLOR_PRIMARY, COLOR_SCROLLBAR, COLOR_SCROLLBAR_HOVER, COLOR_SUCCESS
...
class GeneralView(DebouncedResponsiveMixin, ctk.CTkFrame):
    ...
    def _layout_fields(parent: ctk.CTkFrame, controls: list[ctk.CTkBaseClass], columns: int) -> None:
        for column in (0, 1):
            parent.grid_columnconfigure(
                ...
                uniform="general_reflow" if columns > 1 else "",
            )
    ...
    def _status_panel(self, row: int, title_key: str, values: list[tuple[str, str]]) -> None:
        ...
        for item_row, (label_key, value) in enumerate(values):
            value_color = COLOR_SUCCESS if label_key == "general.status" else None
            status = StatusRow(
                panel.body,
                t(label_key),
                value,
                **({"value_color": value_color} if value_color else {}),
            )
